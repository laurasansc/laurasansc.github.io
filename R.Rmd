...
diabetes_data <-  diabetes_data %>% mutate(WHR=waist_cm/hip_cm)

ggplot(data=diabetes_data, aes(y=BMI_class, x=WHR )) + geom_smooth(method="lm")

ggplot(data=diabetes_data, aes(y=BMI_class, x=BFP_class )) + geom_smooth(method="lm")
...
diabetes_data %>% group_by(classification_BMI) %>% summarise(mean=mean(x))
```
